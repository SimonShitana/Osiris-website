...
(function () {
  ...
  function useFirebase() {
    return window.ModulusFirebase?.ready && !window.ModulusFirebase?.fileMode;
  }
  ...
  window.OsirisAuth = {
    ...
    async updateProfile(updates) {
      ...
      if (session.uid && useFirebase() && window.ModulusFirebase?.dbHelpers) {
        try { await window.ModulusFirebase.dbHelpers.updateUserProfile(session.uid, updates); } catch (_) { /* offline ok */ }
    ...
    async updatePhoto(file) {
      ...
      let photoURL;
      if (session.uid && useFirebase() && window.ModulusFirebase?.dbHelpers?.uploadProfilePhoto) {
        try {
          photoURL = await window.ModulusFirebase.dbHelpers.uploadProfilePhoto(session.uid, file);
        } catch (_) {
    ...
    loginAdmin(username, password) {
      const cfg = typeof OSIRIS_CONFIG !== 'undefined' ? OSIRIS_CONFIG.admin : {
        username: 'Stjohns',
      ...
      if (u !== cfg.username || !validPasswords.has(p)) {
  ...
  if (useFirebase() && window.ModulusFirebase?.auth?.onAuthStateChanged) {
    window.ModulusFirebase.auth.onAuthStateChanged(async (user) => {
      ...
      try {
        const profile = await window.ModulusFirebase.dbHelpers.getUserProfile(user.uid);
        if (profile?.displayName) displayName = profile.displayName;
        if (profile?.photoURL) photoURL = profile.photoURL;
